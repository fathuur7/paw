<?php
$conn = mysqli_connect("localhost", "root", "fathur123", "resto");
...
function register($data)
{
  ...
  // Tentukan level berdasarkan password
  $level = 3; // Default level guest
  
  if($data["password1"] === "ownerResto"){
    $level = 1; // Admin
  } else if ($data["password1"] === "kasirResto"){
    $level = 2; // Kasir
  } else if ($data["password1"] === "accountanResto"){
    $level = 4; // Manager/Accountant
  } else if ($data["password1"] === "kokiResto"){
    $level = 5; // Chef
  }
